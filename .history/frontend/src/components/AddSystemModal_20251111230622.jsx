...
export default function AddSystemModal({ isOpen, onClose, onSystemCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Submitting form with data:", { name, description, file });

      const formDataToSend = new FormData();
      formDataToSend.append("name", name); // use the actual state
      formDataToSend.append("description", description);
      formDataToSend.append("technicalSheet", file); // only once!

      await addSystem(formDataToSend);

      onClose();
      onSystemCreated?.(); // notify parent to refresh
    } catch (error) {
      console.error(
