...
export default function EditSheetModal({ sheet, onClose, onUpdate }) {
  ...
  const [systemName, setSystemName] = useState(sheet.systemName);
  const [instrumentName, setInstrumentName] = useState(sheet.instrumentName);

  const handleGetSheetDetails = async () => {
    const data = await getsheetWithInstrumentAndSystem(sheet.id);
    setInstrumentName(data.instrument.name);
    setSystemName(data.system.name);
  };
  useEffect(() => {
    handleGetSheetDetails();
  }, [sheet.id]);
