...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Get the original file path and generate a PDF file path
...
router.get("/:id/download", async (req, res) => {
  try {
    ...
    console.log("Audit log called for register technical sheet download");
    await AuditLog.create({
      userId: req.user.id, // user performing the action (from JWT)
      action: "DOWNLOAD",
      userLogged: req.user.username,
...
///////////////////////////////////////////////
//get sheet with instrument and system data
// /api/sheetdata/:technicalSheetId
router.get("/api/sheetdata/:id", async (req, res) => {
  try {
    const technicalSheet = await TechnicalSheet.findByPk(req.params.id, {
      include: [
        {
          model: Instrument,
          include: [System],
    ...
    if (!technicalSheet) return res.status(404).json({ error: "Not found" });

    res.json(technicalSheet);
  } catch (err) {
    console.error("Error fetching technical sheet:", err);
    res.status(500).json({ error: err.message });
  }
});
