...
const path = require("path"); // for the storage function
// ✅ Get own profile
...
// Get the size of the upload folder
router.get("/upload-folder-size", async (req, res) => {
  try {
    // Construct the absolute path to your uploads folder
    const uploadsPath = path.join(process.cwd(), "../uploads/technical-sheets");

    // Get the size in bytes
    const sizeInBytes = await getFolderSize(uploadsPath);
    ...
    res.json({
      ...
      sizeInMB: parseFloat(sizeInMB.toFixed(2)), // Rounds to 2 decimal places
    });
  } catch (error) {
    console.error("Error calculating folder size:", error);
    res.status(500).json({
      success: false,
      message: "Could not calculate folder size.",
      error: error.message,
    });
  }
});
