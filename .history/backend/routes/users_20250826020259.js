...
const path = require("path"); // for the storege function
// ✅ Get own profile
...
//
router.get("/upload-folder-size", (req, res) => {
  try {
    const uploadsDir = path.join(__dirname, "../uploads"); // adjust if needed
    console.log("Checking uploads dir:", uploadsDir);

    if (!fs.existsSync(uploadsDir)) {
      console.log("Uploads folder not found");
      return res.json({ sizeMB: 0 });
    }
