...
// GET /api/instruments/by-service
router.get(
  "/by-service",
  authenticateToken, // user info is in req.user
  async (req, res) => {
    ...
    try {
      ...
      if (!userService) {
        return res
          .status(400)
          .json({ message: "User has no service assigned." });
      }

      // Find instruments that include the user's service
